...
import { Activity, Plus, Camera, Settings, Sparkles, AlertTriangle, CheckCircle2, X, LogIn, ShieldCheck, Layers } from 'lucide-react';
...
export default function App() {
  ...
  // Dynamic Plugin & Metrics state
  const [plugins, setPlugins] = useState([]);
  const [metricsSummary, setMetricsSummary] = useState({});
  const [metricEntries, setMetricEntries] = useState([]);
  const [entries, setEntries] = useState([]); // legacy weight/steps for charts
  ...
  // Modals state
  const [dynamicLogOpen, setDynamicLogOpen] = useState(false);
  const [selectedPluginId, setSelectedPluginId] = useState('weight');
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [pluginsRes, summaryRes, entriesRes, statsRes, goalsRes, jobsRes] = await Promise.all([
        fetch('/api/plugins'),
      ...
      if (summaryRes.status === 401 || entriesRes.status === 401) {
        handleLogout();
        return;
      }
      ...
      if (pluginsData.success) setPlugins(pluginsData.plugins || []);
      if (summaryData.success) setMetricsSummary(summaryData.summary || {});
      ...
      // Fetch all entries for all plugins
      if (pluginsData.plugins) {
        const allEntriesPromises = pluginsData.plugins.map(p =>
          fetch(`/api/metrics/${p.id}/entries`, { headers }).then(r => r.json())
        );
        const allResults = await Promise.all(allEntriesPromises);
        const combined = [];
        allResults.forEach(r => {
          if (r.success && r.entries) {
            combined.push(...r.entries);
          }
        });
        combined.sort((a, b) => b.date.localeCompare(a.date));
        setMetricEntries(combined);
      }
  ...
  const handleDeleteMetricEntry = async (entryId) => {
    const entry = metricEntries.find(e => e.id === entryId);
    if (!entry) return;
    if (!window.confirm(`Are you sure you want to delete this ${entry.metric_id} entry?`)) return;

    try {
      const res = await fetch(`/api/metrics/${entry.metric_id}/entries/${entryId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
          ...
          <div className="container header-container">
            <div className="logo-section">
              ...
              <div>
                ...
                <p className="logo-subtitle">Extensible Multi-User Health & Activity Platform</p>
              </div>
            </div>
        ...
        <main className="container" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 20px' }}>
          <div style={{
            ...
            <p style={{ color: 'var(--text-secondary)', fontSize: '14px', lineHeight: 1.6, marginBottom: '24px' }}>
              Sign in with your password or one-touch <strong>Biometrics (Face ID / Touch ID)</strong> to securely track your weight, steps, and equipment sessions.
            </p>
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <header className="header">
        <div className="container header-container">
          <div className="logo-section">
            ...
            <div>
              <h1 className="logo-title">HealthPulse</h1>
              <p className="logo-subtitle">Extensible Activity & Gear Tracker</p>
            </div>
          </div>
          ...
          <div className="header-actions">
            ...
            <button
              className="btn btn-primary"
              onClick={() => { setSelectedPluginId('weight'); setDynamicLogOpen(true); }}
            >
              <Plus size={16} />
              Log Entry
            </button>
            ...
            >
              <Settings size={16} />
              <span>{currentUser?.username || 'Settings'}</span>
            </button>
          </div>
        </div>
      </header>
      ...
      <main className="container" style={{ flex: 1, paddingBottom: '60px' }}>
          ...
          <div style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                {bannerAlert.error || 'The numbers on the scale display were not legible.'} Your most recent valid entry <strong>({stats?.latest_weight ? `${stats.latest_weight} ${stats?.weight_unit || 'lbs'}` : 'recorded entry'})</strong> is retained.
              </div>
          ...
          <div className="metrics-grid">
            {plugins.map(p => (
              <DynamicMetricCard
                key={p.id}
                plugin={p}
                data={metricsSummary[p.id]}
                onOpenLogModal={(pluginId) => {
                  setSelectedPluginId(pluginId);
                  setDynamicLogOpen(true);
                }}
              />
            ))}
          </div>
        ...
        {/* Dynamic Multi-Metric History Table */}
        <section style={{ marginTop: '24px' }}>
          <DynamicHistoryTable
            plugins={plugins}
      ...
      {/* Dynamic Multi-Metric Log Modal */}
      <DynamicLogModal
        isOpen={dynamicLogOpen}
        onClose={() => setDynamicLogOpen(false)}
        plugins={plugins}
        initialMetricId={selectedPluginId}
